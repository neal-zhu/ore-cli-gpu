...
async fn main() {
    ...
    let exec_cluster = args.exec_rpc.unwrap_or(cli_config.json_rpc_url);

    let default_keypair = args.keypair.unwrap_or(cli_config.keypair_path);
    let rpc_client = RpcClient::new_with_commitment(cluster, CommitmentConfig::confirmed());
    let exec_client =  RpcClient::new_with_commitment(exec_cluster, CommitmentConfig::confirmed());
...
impl Miner {
    pub fn new(rpc_client: Arc<RpcClient>,exec_client: Arc<RpcClient>, priority_fee: u64, keypair_filepath: Option<String>) -> Self {
        Self {
            rpc_client,
            exec_client,
            keypair_filepath,
